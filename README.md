...
- 运维能力：任务健康、观测告警、Webhook 告警通知、操作审计、运行日志、系统维护、配置导入导出、自动备份、端到端 smoke 验收。
...
保存后先点击“测试”。测试通过后再用于同步任务。需要多人共用时，可以由管理员或连接创建者设置共享。
...
1. 点击“新建任务”。
2. 选择源连接和源表。
3. 选择目标 Teable 连接、Base 和目标表。
4. 配置字段映射，优先使用智能匹配结果，确认被跳过的不兼容字段。
5. 选择同步模式和写入策略。
6. 需要调整分页、写入批量、初始化保护、增量策略、双向同步或删除同步时，展开“高级设置”。
7. 保存前建议在“更多”里执行“预检”和“预览数据”，确认无阻断问题后再运行。
...
- 手动任务：点击“同步”。
- 定时/准实时任务：点击“启动调度”，调度器会按配置间隔执行；运行中的任务可以取消。
- 任务列表常驻核心操作：详情、同步、启动/停止调度、更多。
- “更多”里包含预览数据、预检、字段变更、一致性校验、近期日志、失败批次、复制、编辑和删除等低频操作。
- 任务详情页分为“概览 / 运行历史 / 高级信息 / 近期日志”。配置摘要、字段映射和字段快照都在“高级信息”里。
- 一致性校验用于只读对比源端和目标端，日期型字段会按业务日期归一化后比较。
- 如果出现失败批次，可以从“更多 -> 失败批次”进入重试或清理。

### 5. 观测告警和告警推送

“观测告警”页面现在定位为异常处理队列，第一屏重点显示：
...
任务健康默认只显示异常任务，需要巡检时可点击“查看全部”。最近警告/错误日志默认折叠，完整排查请到“日志”页面。

管理员可以点击“设置通知”启用 Webhook 告警通知：
...
配置迁移入口在“系统维护 -> 高级维护 -> 环境迁移”。管理员可以导出不含密钥的迁移包、预检迁移包并导入；含密钥迁移包只允许系统初始管理员导出。
...
- “日志”页面以同步执行流为主，用于排查同步过程；操作审计默认折叠，仅在排查用户、连接、任务和系统操作时展开。
- “系统维护”第一屏显示系统状态和检查项，会检查配置加密、数据目录写入、配置解密、连接/任务引用完整性和待处理失败批次。
- “系统维护”的配置备份默认折叠；高级维护里包含上线验收、环境迁移和测试环境整理等管理员低频操作。
